#!/usr/bin/env bash
#
# Verify doc/bemtvi-diff.txt is up to date with the Markdown it is generated
# from. Regenerates it and fails if the result differs from what is committed — so a
# push can't ship a help file that no longer matches doc/bemtvi-diff.md. Wired
...
OUTPUT="doc/bemtvi-diff.txt" # must match OUTPUT in gen-vimdoc.sh
