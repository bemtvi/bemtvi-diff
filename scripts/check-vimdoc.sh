#!/usr/bin/env bash
#
# Verify doc/nxvim-diff.txt is up to date with the Markdown it is generated
# from. Regenerates it and fails if the result differs from what is committed — so a
# push can't ship a help file that no longer matches doc/nxvim-diff.md. Wired
...
OUTPUT="doc/nxvim-diff.txt" # must match OUTPUT in gen-vimdoc.sh
