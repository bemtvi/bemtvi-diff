...
-- mirror — through the real editor. Run with `bemtvi --test-plugin`.
...
-- Read through btv.win.call so it resolves against THAT pane's window, focused or not.
local function view_of(pane)
  return btv.win.call(pane.view:winid(), btv.win.saveview)
end
...
btv.test.describe("bemtvi-diff scroll/cursor sync", function()
  ...
  btv.test.it("disables scroll animation on the panes (so a synced scroll can't desync)", function()
    local s = open_tall()
    ...
    for _, p in ipairs(s.panes) do
      btv.test.expect(vim.wo[p.view:winid()].scrollanim).to_be(false)
    end
  end)

  btv.test.it("mirrors a vertical scroll onto the other pane", function(t)
    local s = open_tall()

    -- Both panes start pinned to the top.
    btv.test.expect(view_of(s.panes[1]).topline).to_be(1)
    btv.test.expect(view_of(s.panes[2]).topline).to_be(1)

    -- Scroll the focused (left) pane to the bottom; the right pane follows.
    t:feed("G", { settle = 2 })
    local top = view_of(s.panes[1]).topline
    btv.test.expect(top > 1).to_be(true) -- it really scrolled off the top
    btv.test.expect(view_of(s.panes[2]).topline).to_be(top)
  ...
  btv.test.it("mirrors the cursor line onto the other pane", function(t)
    local s = open_tall()

    t:feed("30G", { settle = 2 })
    btv.test.expect(view_of(s.panes[1]).lnum).to_be(30)
    btv.test.expect(view_of(s.panes[2]).lnum).to_be(30)
  end)

  btv.test.it("respects sync_scroll = false (panes scroll independently)", function(t)
    require("bemtvi-diff").setup({ sync_scroll = false })
    local s = open_tall()

    t:feed("G", { settle = 2 })
    btv.test.expect(view_of(s.panes[1]).topline > 1).to_be(true)
    -- The right pane was never touched.
    btv.test.expect(view_of(s.panes[2]).topline).to_be(1)
  end)

  btv.test.it("detaches the sync autocmds on close (a later scroll is inert)", function(t)
    open_tall()
    local sess = diff.session()
    btv.test.expect(type(sess._detach)).to_be("function")
