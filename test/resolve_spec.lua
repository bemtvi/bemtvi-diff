...
-- the surrounding context untouched). Run with `bemtvi --test-plugin .`.
...
local function open_conflict(t)
  local path = btv.test.tempdir() .. "/merge.txt"
  btv.await(btv.fs.write(path, table.concat(CONFLICT, "\n") .. "\n"))
  t:cmd("edit " .. path)
  return path, t:buf()
end
...
local function await_lines(t, bufnr, expected)
  t:wait_for(function()
    return table.concat(btv.buf.lines(bufnr, 0, -1), "\n") == table.concat(expected, "\n")
  end, { tries = 300, interval = 10, message = "buffer never reached the resolved content" })
  return btv.buf.lines(bufnr, 0, -1)
end

btv.test.describe("bemtvi-diff conflict resolve", function()
  ...
  btv.test.it("threads the resolve target (live buf + absolute marker lines)", function(t)
    local _, bufnr = open_conflict(t)
    diff.conflict()
    local s = await_ready()
    btv.test.expect(s.resolve ~= nil).to_be(true)
    btv.test.expect(s.resolve.buf).to_be(bufnr)
    local r = s.resolve.regions[1]
    -- Absolute buffer lines: <<< is line 2, >>> is line 6 in the file above.
    btv.test.expect(r.first).to_be(2)
    btv.test.expect(r.last).to_be(6)
    btv.test.expect(table.concat(r.ours, "|")).to_be("our change")
    btv.test.expect(table.concat(r.theirs, "|")).to_be("their change")
  end)

  btv.test.it("refresh re-parses the live buffer, not the opening snapshot", function(t)
    local _, bufnr = open_conflict(t)
    diff.conflict()
    local s = await_ready()
    btv.test.expect(type(s.spec.reload)).to_be("function")
    ...
    btv.await(btv.buf.set_lines(bufnr, 2, 3, true, { "our EDITED change" }))
    diff.refresh()
    t:wait_for(function()
    ...
    btv.test
      .expect(table.concat(btv.buf.lines(diff.session().panes[1].view:bufnr(), 0, -1), "|"))
      .to_be("top|our EDITED change|bot")
  end)

  btv.test.it("choose_ours replaces the marker block with our side", function(t)
    local _, bufnr = open_conflict(t)
    diff.conflict()
    local s = await_ready()
    nav.choose_ours(s)
    local lines = await_lines(t, bufnr, { "top", "our change", "bot" })
    btv.test.expect(table.concat(lines, "|")).to_be("top|our change|bot")
    -- The diff tab is gone (resolve closes it) — back to a single window on the file.
    btv.test.expect(diff.session()).to_be_nil()
  end)

  btv.test.it("choose_theirs replaces the marker block with their side", function(t)
    local _, bufnr = open_conflict(t)
    diff.conflict()
    local s = await_ready()
    nav.choose_theirs(s)
    local lines = await_lines(t, bufnr, { "top", "their change", "bot" })
    btv.test.expect(table.concat(lines, "|")).to_be("top|their change|bot")
  end)

  btv.test.it("choose_both keeps both sides — ours then theirs", function(t)
    local _, bufnr = open_conflict(t)
    diff.conflict()
    local s = await_ready()
    nav.choose_both(s)
    local lines = await_lines(t, bufnr, { "top", "our change", "their change", "bot" })
    btv.test.expect(table.concat(lines, "|")).to_be("top|our change|their change|bot")
  end)

  btv.test.it("pick_lines + apply_picked composes a resolution from either side", function(t)
    local _, bufnr = open_conflict(t)
    diff.conflict()
    local s = await_ready()
    ...
    local function pick_from(pane_idx)
      ...
      btv.await(btv.wait_for(function()
        return btv.win.current() == s.panes[pane_idx].view:winid() and s:cursor_row() == row
    ...
    btv.test.expect(table.concat(text, "|")).to_be("their change|our change")
    -- Each pick recorded the pane it came from (for the gutter sign): theirs = side "b",
    -- ours = side "a" (a 2-way conflict is ours | theirs).
    btv.test.expect(picks[1].side).to_be("b")
    btv.test.expect(picks[2].side).to_be("a")

    nav.apply_picked(s)
    local lines = await_lines(t, bufnr, { "top", "their change", "our change", "bot" })
    btv.test.expect(table.concat(lines, "|")).to_be("top|their change|our change|bot")
  end)

  btv.test.it("pick_lines refuses lines outside the conflict (shared context)", function(t)
    open_conflict(t)
    diff.conflict()
    local s = await_ready()
    ...
    btv.await(btv.wait_for(function()
      return btv.win.current() == s.panes[1].view:winid() and s:cursor_row() == 1
    ...
    btv.test.expect(msg:match("aren't part of the conflict") ~= nil).to_be(true)
    btv.test.expect(s.resolve.regions[1].picks == nil or #s.resolve.regions[1].picks == 0).to_be(true)
  end)

  btv.test.it("apply_picked with nothing staged notifies and writes nothing", function(t)
    local _, bufnr = open_conflict(t)
    diff.conflict()
    local s = await_ready()
    nav.apply_picked(s)
    ...
    btv.test.expect(msg:match("no lines picked") ~= nil).to_be(true)
    -- The diff is still open and the buffer still holds its markers (nothing was written).
    btv.test.expect(diff.session() ~= nil).to_be(true)
    btv.test.expect(btv.buf.lines(bufnr, 1, 2)[1]).to_be("<<<<<<< HEAD")
  end)

  btv.test.it("clear_picked discards what pick_lines staged", function(t)
    ...
    nav.clear_picked(s)
    btv.test.expect(region.picks).to_be_nil()
  end)

  btv.test.it("resolves only the conflict under the cursor in a multi-conflict file", function(t)
    local content = {
      "top",
      "<<<<<<< HEAD",
      "ours1",
    ...
    local path = btv.test.tempdir() .. "/multi.txt"
    btv.await(btv.fs.write(path, table.concat(content, "\n") .. "\n"))
    t:cmd("edit " .. path)
    local bufnr = t:buf()
    diff.conflict()
    local s = await_ready()
    btv.test.expect(#s.resolve.regions).to_be(2)
  ...
  btv.test.it("choose_* on a non-conflict diff is a no-op notice", function(t)
    ...
    local s = await_ready()
    btv.test.expect(s.resolve).to_be_nil()
    ...
    btv.test.expect(msg:match("nothing to resolve") ~= nil).to_be(true)
  end)
end)
